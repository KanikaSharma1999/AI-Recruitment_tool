...
const MiniBar = ({ value, color, max = 100 }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
    ...
    <span style={{ fontSize: 12, fontWeight: 800, minWidth: 36, textAlign: 'right', color }}>{Math.round(val(value))}%</span>
  </div>
);
...
const Delta = ({ a, b }) => {
  ...
  return <span style={{ fontSize: 11, fontWeight: 700, color: d > 0 ? '#059669' : '#dc2626' }}>{d > 0 ? `+${d}` : d}</span>;
};

const Chip = ({ label, color = '#6366f1', bg = '#ede9fe' }) => (
  <span style={{ padding: '2px 8px', borderRadius: 999, fontSize: 10, fontWeight: 700, background: bg, color, marginRight: 4, marginBottom: 4, display: 'inline-block' }}>{label}</span>
);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {jobs.map(j => (
        ...
        >
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 800, fontSize: 15, color: '#1e293b' }}>{j.title}</div>
            <div style={{ fontSize: 12, color: '#64748b', marginTop: 2 }}>{j.candidate_count ?? ''} candidates · {j.department || 'General'}</div>
          </div>
          <div style={{ fontSize: 12, color: '#6366f1', fontWeight: 700 }}>Select →</div>
        </button>
      ))}
    </div>
  );
...
function CandidateSelector({ candidates, selected, onToggle }) {
  const recColor = { 'Strong Hire': '#059669', 'Hire': '#2563eb', 'Hold': '#d97706', 'Reject': '#dc2626' };
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 14 }}>
      ...
      {candidates.map((c, i) => {
        ...
        const rec = c.hiring_summary?.recommendation;
        return (
          <div key={c.id} onClick={() => onToggle(c.id)}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: isSelected ? PALETTE[rank] : '#e5e7eb', display: 'flex', alignItems: 'center', justifyContent: 'center', color: isSelected ? '#fff' : '#64748b', fontWeight: 800, fontSize: 15, flexShrink: 0 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ fontSize: 22, fontWeight: 800, color: isSelected ? PALETTE[rank] : '#475569' }}>{Math.round(c.score || 0)}%</span>
              {rec && <span style={{ fontSize: 10, fontWeight: 700, color: recColor[rec] || '#64748b', background: '#f1f5f9', padding: '2px 7px', borderRadius: 999 }}>{rec}</span>}
            </div>
          </div>
        );
      })}
...
function CompareTable({ candidates }) {
  const rows = [
    { label: '🎯 AI Match Score', render: (c, i) => <MiniBar value={c.score} color={PALETTE[i]} /> },
    { label: '📅 Experience', render: c => <span style={{ fontWeight: 700, fontSize: 13 }}>{val(c.experience_years, 0).toFixed(1)} years</span> },
    ...
    { label: '🤖 AI Recommendation', render: c => <RecBadge rec={c.hiring_summary?.recommendation} /> },
    ...
    { label: '⚠️ Weaknesses', render: c => <div>{(c.hiring_summary?.weaknesses || []).slice(0, 3).map((w, i) => <div key={i} style={{ fontSize: 12, color: '#dc2626', marginBottom: 2 }}>• {w}</div>)}{!c.hiring_summary?.weaknesses?.length && <span style={{ color: '#94a3b8', fontSize: 12 }}>—</span>}</div> },
    ...
    { label: '📊 Interview Status', render: c => <span style={{ fontSize: 12, fontWeight: 700 }}>{(c.interview?.status || c.status || '—').replace(/_/g, ' ')}</span> },
  ];
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
        <thead>
          <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
            <th style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 700, fontSize: 11, color: '#64748b', width: 180, textTransform: 'uppercase', letterSpacing: '0.5px' }}>METRIC</th>
            {candidates.map((c, i) => (
              <th key={c.id} style={{ padding: '12px 16px', textAlign: 'left', fontWeight: 800, fontSize: 13, color: PALETTE[i], borderLeft: `3px solid ${PALETTE[i]}` }}>
                <div>{c.name}</div>
...
function RecruiterInsights({ candidates, aiInsights, loadingInsights }) {
  ...
  const insights = [
    ...
    { icon: '🏆', label: 'Best Overall Hire', winner: best(c => val(c.score)), metric: c => `${Math.round(val(c.score))}% match` },
  ];

  const overallBest = best(c => val(c.score));
  const secondBest = candidates.filter(c => c.id !== overallBest.id).sort((a, b) => val(b.score) - val(a.score))[0];
  ...
  return (
    <div style={{ background: 'linear-gradient(135deg, #1e293b, #0f172a)', borderRadius: 16, padding: 28, marginBottom: 24, color: '#fff' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 22 }}>
        ...
        <div style={{ fontWeight: 800, fontSize: 16 }}>AI Recruiter Insights</div>
        ...
        {insights.map((ins, i) => {
          ...
          return (
            <div key={i} style={{ background: '#1e293b', borderRadius: 12, padding: '14px 16px', border: `1px solid ${PALETTE[winnerIdx] || '#334155'}44` }}>
              ...
              <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>{ins.label}</div>
              <div style={{ fontWeight: 800, fontSize: 14, color: PALETTE[winnerIdx] || '#fff' }}>{ins.winner?.name || '—'}</div>
...
export default function Compare() {
  ...
  const handleCompare = async () => {
    ...
    try {
      ...
      const url = `${API.defaults.baseURL || 'http://localhost:8000'}/candidates/compare-insights?ids=${selectedIds.join(',')}`;
  ...
  const StepBadge = ({ n, label, active, done }) => (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 13, background: done ? '#10b981' : active ? '#6366f1' : '#e2e8f0', color: done || active ? '#fff' : '#94a3b8' }}>
        {done ? '✓' : n}
      </div>
      <span style={{ fontSize: 13, fontWeight: active ? 700 : 500, color: active ? '#1e293b' : '#94a3b8' }}>{label}</span>
    </div>
  );
      ...
      <div className="main-content">
        ...
        <div className="page-body animate-fade">
          ...
          <div style={{ marginBottom: 24 }}>
            <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1e293b' }}>Enterprise Candidate Comparison</h1>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <button onClick={handleCompare} disabled={selectedIds.length < 2 || loading}
                    style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 22px', background: selectedIds.length >= 2 ? '#6366f1' : '#e2e8f0', color: selectedIds.length >= 2 ? '#fff' : '#94a3b8', border: 'none', borderRadius: 10, fontWeight: 700, fontSize: 13, cursor: selectedIds.length >= 2 ? 'pointer' : 'not-allowed' }}>
            ...
            <div className="animate-fade">
              ...
              <div style={{ background: '#fff', borderRadius: 16, border: '1px solid #e2e8f0', overflow: 'hidden' }}>
                <div style={{ padding: '18px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontWeight: 800, fontSize: 15, color: '#1e293b' }}>Side-by-Side Enterprise Analysis</span>
                </div>
